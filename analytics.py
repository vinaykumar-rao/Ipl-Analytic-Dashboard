...
def prepare(matches, deliveries):
    ...
    required_d = {'match_id', 'inning', 'over', 'batter', 'bowler', 'batsman_runs',
                  'total_runs', 'extra_runs', 'extras_type', 'dismissal_kind', 'is_wicket',
                  'batting_team', 'bowling_team'}
    for frame, required, label in [(m, required_m, 'matches.csv'), (d, required_d, 'deliveries.csv')]:
        missing = required - set(frame.columns)
        if missing:
            raise ValueError(f'{label} is missing columns: {", ".join(sorted(missing))}. Use the linked Kaggle dataset.')
        if frame.empty:
    ...
    # Calendar year correctly maps 2007/08 to 2008 and 2020/21 to 2020.
    ...
    if not d.match_id.isin(m.id).all():
        raise ValueError('Some deliveries have no matching match ID. Upload a matching CSV pair.')
    ...
    for frame, columns in [(m, ['team1', 'team2', 'winner', 'toss_winner']),
    ...
    # A no-ball counts as a ball faced by the batter; neither a wide nor a no-ball is a legal bowling ball.
...
def filter_data(m, d, seasons, team='All teams'):
    ...
    if team != 'All teams':
        selected = selected[selected.team1.eq(team) | selected.team2.eq(team)]
    # Super overs are excluded from player/league career statistics.
    balls = d[d.match_id.isin(selected.id) & d.inning.le(2)]
